...
def create_model(no_in=None, no_h1=None, no_h2=None, no_out=None):
    ...
    model.add(keras.layers.Dense(no_h1, input_shape=(no_in, ), activation='tanh', kernel_initializer='random_uniform', bias_initializer='ones'))
    model.add(keras.layers.Dense(no_h2, activation='tanh', kernel_initializer='random_uniform', bias_initializer='ones'))
...
def train(trainfile, testfile, model):
    ...
    for i, (train_index, test_index) in enumerate(KFold(3).split(train_x)):
        ...
        model.fit(x_train, y_train,
                   batch_size=10, epochs=20, verbose=1, shuffle=True, validation_data=(x_value, y_value))
    ...
    print('Final result: ')
    print('Average in-sample loss:', statistics.mean(in_sample_loss), ' Average in-sample accuracy:', statistics.mean(in_sample_accur))
...
if __name__== "__main__":
    ...
    print('\nModel [256, 6, 2, 1] ')
    model1 = create_model(no_in=256, no_h1=6, no_h2=2, no_out=10)
